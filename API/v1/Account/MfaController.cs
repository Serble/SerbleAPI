...
public class MfaController(ITokenService tokens, IUserRepository userRepo) : ControllerManager {
    ...
    public IActionResult Authenticate([FromBody] MfaAuthBody body) {
        ...
        if (!tokens.ValidateFirstStepLoginToken(body.LoginToken, out User user))
    ...
    public IActionResult CheckTotp([FromBody] MfaAuthBody body) {
        User? user = HttpContext.User.GetUser(userRepo);
        if (user == null) return Unauthorized();
        bool valid = user.ValidateTotp(body.TotpCode);
        return Ok(new { success = true, valid });
    }

    [HttpGet("totp/qrcode")]
    [Authorize(Policy = "UserOnly")]
    public IActionResult GetTotpQrCode() {
